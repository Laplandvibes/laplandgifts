import type { BoutiqueCopy } from './en'

const copy: Record<string, BoutiqueCopy> = {
  'lauri-handicrafts': {
    description: '自 1924 年以来手工制作的刀具、饰品和毛毡制品。驯鹿骨、卷纹桦木与芬兰钢。',
    tags: ['刀具', '手工艺品'],
  },
  'marttiini': {
    description: '自 1928 年以来享誉世界的刀具。工厂直销店与品牌专卖店。免费刻字。',
    tags: ['刀具', '礼物'],
  },
  'pentik': {
    description: '自 1971 年起在波西奥运营的世界最北陶瓷工厂。陶瓷、家居装饰与设计。',
    tags: ['陶瓷', '家居装饰'],
  },
  'duodji-shop': {
    description: '位于 Sajos 文化中心。芬兰种类最齐全的正宗萨米手工艺品。',
    tags: ['萨米工艺品', '杜奥德吉'],
  },
  'samekki': {
    description: '手工制作的萨米银饰：戒指、胸针和传统杜奥吉(duodji)作品。',
    tags: ['银饰', '萨米工艺品'],
  },
  'piece-of-lapland': {
    description: '纪念品、萨米工艺品、木雕,以及极光主题商品。',
    tags: ['纪念品', '手工艺品'],
  },
  'rovaniemi-souvenirs-shop': {
    description: '在自家工作坊手工制作的驯鹿角制品。免费刻字。北极圈。',
    tags: ['驯鹿角', '手工艺品'],
  },
  'christmas-house-shop': {
    description: '圣诞老人村最大的纪念品商店：拉普兰商品、饰品、美食与证书。',
    tags: ['纪念品', '美食'],
  },
  'korundi-shop': {
    description: '在 Korundi 文化中心欣赏并选购北极艺术与设计。独特礼物与高端商品。',
    tags: ['艺术', '设计'],
  },
  'shoppi-craft-design': {
    description: '位于莱维滑雪坡脚下,芬兰工匠制作的饰品、配饰与纪念品。',
    tags: ['饰品', '设计'],
  },
  'siida-shop': {
    description: '萨米博物馆商店：杜奥吉(duodji)手工艺品、饰品,以及以拉普兰自然为灵感的礼物。',
    tags: ['杜奥德吉', '礼物'],
  },
  'tankavaaran-kultakyla': {
    description: '来自拉普兰淘金传统中心的金饰、真金与矿石。',
    tags: ['黄金', '矿物'],
  },
  'mailan-putiikki': {
    description: '位于 Äkäslompolo 十字路口的礼品与纪念品专门店，是 Ylläs 同类中最大的一家。1970 年创立。',
    tags: ['礼品', '纪念品'],
  },
  'kuukkeli-shop': {
    description: '位于 Saariselkä 购物中心的纪念品、拉普兰美味与纺织品商店。自制浆果制品：果酱、果冻、酱汁与果汁。',
    tags: ['美味', '纪念品'],
  },
  'mariellen-vaatehuone': {
    description: 'Enontekiö 的服装店，经营 Anar、Nanso、Rukka 与 Luhta，另有饰品与版画。',
    tags: ['服装', '饰品'],
  },
  'lahjapuoti-tiinuska': {
    description: '来自罗瓦涅米的礼品店，1987 年创立。Aarikka 与姆明商品、饰品、纺织品与零食，可在网店选购。',
    tags: ['设计', '礼品'],
  },
  'lappi-shop-levi': {
    description: 'Lappituote 位于 Levi 市中心的门店：纪念品、野味与浆果罐头、驯鹿角制品、芬兰传统刀与木杯。',
    tags: ['手工艺品', '纪念品'],
  },
  'utsjoki-handicraft': {
    description: '由 Juha Reinola 用北方的天然材料手工制作：驯鹿角、卷纹桦木、枯立松、驯鹿皮与桦树皮。',
    tags: ['手工艺品', '驯鹿角'],
  },
}

export default copy
